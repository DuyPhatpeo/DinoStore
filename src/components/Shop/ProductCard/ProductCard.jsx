...
export default function ProductCard({
  ...
  const handleAddToCart = (e) => {
    e.stopPropagation(); // Ngăn chuyển trang khi bấm nút
    if (!selectedSize) {
    ...
    <div className="overflow-hidden bg-white rounded group transition">
      ...
      <Link to={`/product/${id}`}>
        <div className="relative w-full aspect-[4/5] bg-gray-100 overflow-hidden cursor-pointer">
          ...
          <div
            ...
            className="absolute z-10 flex flex-col p-1 transition-all duration-300 translate-x-4 bg-white rounded-md shadow opacity-0 cursor-pointer top-2 right-2 group-hover:opacity-100 group-hover:translate-x-0"
          >
      ...
      <div className="p-3 text-center space-y-2">
        ...
        {normalizedSizes.length > 0 && (
          <div
            className="flex justify-center flex-wrap gap-2 mb-1"
            onClick={(e) => e.stopPropagation()}
          >
        ...
        {/* Tên và giá */}
        <Link to={`/product/${id}`} className="block space-y-1">
        ...
        {/* Nút Select Options hoặc Add to Cart */}
        {selectedSize ? (
          <button
          ...
          <Link to={`/product/${id}`}>
            <button
              onClick={(e) => e.stopPropagation()}
              className="mt-1 text-[13px] font-medium bg-white text-black border border-gray-400 px-3 py-1 rounded"
            >
              SELECT OPTIONS
            </button>
          </Link>
